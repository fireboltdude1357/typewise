...
| Save fidelity | The original local/cloud payload did not identify roster semantics. | Persist scope in local drafts and Convex records; restore it before authoritative catalog/move rehydration. Legacy local drafts fall back to National. | Implemented; final Convex deployment and cloud round-trip pending. |
...
| Images | Fourteen catalog forms had Showdown sprite URLs that returned 404. | Retry nearest visual form, base-species Showdown sprite, then PokéAPI official art and base sprite. Retry state is keyed by failed URL. | All 1,379 Gen IX National entries had a successful primary or fallback URL locally; deployed smoke pending. |
| Route caching | Reusing the original 24-hour cache keys allowed old and new response schemas to coexist after deployment. | Version both catalog and move-list cache keys whenever their schemas or semantics change. | Key bump implemented; deployed API smoke pending. |
...
| --- | --- | --- |
| Generator determinism and pinned-source hashes | Generator ran twice byte-identically on 2026-07-10; all four pinned Z-A hashes, source-array checks, generator ESLint, and invariants passed. Current hash and title counts are recorded above. | Green; re-run once in the final clean tree and require no diff. |
...
| `npm run test:e2e` | Windows system Chrome passed 6/6 applicable desktop/mobile flows on 2026-07-10; two credential-gated cloud cases skipped. | Local green; live rerun pending. |
...
| Convex development + production schema/functions | Baseline production deployment `rare-pony-381` is live; it predates the optional/default-National `scope` compatibility field. | **Pending compatibility-first redeploy.** |
| Vercel production smoke | `https://typewise-indol.vercel.app` is live on the baseline build. | **Pending scope build, cache, image, and browser smoke.** |
| Authenticated live save/reload/delete | Passed twice against the baseline Clerk development integration. | Re-run with a scope-aware payload after deployment; Clerk intentionally remains in development mode. |
| Git review | Baseline history is pushed to `fireboltdude1357/typewise`. | Pending logical commits, final diff review, push, and clean-worktree proof. |
...
- [ ] Run Playwright desktop and mobile locally, then against the Vercel URL.
...
- [x] Verify every catalog entry has at least one successful image candidate
      locally; repeat against the deployed API.
- [ ] Deploy Convex development and production schema/functions.
- [ ] Deploy Vercel; smoke both scope-aware API routes and rule out stale cache
      schemas.
- [ ] Repeat authenticated create/list/load/delete with a scope-aware team.
- [ ] Commit in reviewable units, push, confirm CI, and record commit/deployment
